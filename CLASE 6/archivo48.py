"""
 Confeccionar un programa que contenga dos controles de tipo ScrolledText.
  ...
  Mediante 4 controles de tipo Entry indicar desde que fila y columna hasta 
  que fila y columna extraer caracteres del primer ScrolledText y 
  copiarlos al segundo ScrolledText cuando se presione un botón.
"""
...
class Aplicacion:
    ...
    def copiar(self):    # Copiar el texto del primer ScrolledText

        # Obtencion de datos de variables para los limites de ScrolledText
        iniciofila = self.dato1.get()
        iniciocolumna = self.dato2.get()
        finfila = self.dato3.get()
        fincolumna = self.dato4.get()

        # Obtencion de string de ScrolledText
        datos=self.scrolledtext1.get(iniciofila+"."+iniciocolumna, finfila+"."+fincolumna)
        self.scrolledtext2.delete("1.0", tk.END)        # Borramos cualquier texto en el ScrolledText 2 
        self.scrolledtext2.insert("1.0", datos)         # insertamos lo obtenido
